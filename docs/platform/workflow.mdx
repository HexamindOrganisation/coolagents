...
<Note>
**register vs serve.** You rarely call `register` by hand — [`hexgate
serve`](/cli/serve) auto-registers on first run (idempotent; a content-hash
short-circuits no-ops). Use an explicit `hexgate register` for CI or
deliberate-deploy flows, paired with `hexgate serve --no-auto-register` to *fail*
if the agent wasn't pre-registered.
</Note>
