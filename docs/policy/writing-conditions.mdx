...
The engine is [RE2](https://github.com/google/re2/wiki/Syntax), the same one the
compiled policy bundle runs, so a pattern behaves identically wherever the policy
is enforced. RE2 leaves out backreferences and lookaround; a pattern using them
is refused when the policy loads, with the reason RE2 gives. Note that `$` means
the end of the value, and does not also match just before a trailing newline the
way some regex engines do.
</Tip>
