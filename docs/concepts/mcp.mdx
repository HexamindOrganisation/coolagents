...
- **Catalog threat scanning** — typosquatting and prompt-injection
  pattern detection on tool descriptions at registration time is not
  performed.
...
- **Declarative `mcp_servers:` block in `agent.yaml`** — attach servers
  programmatically via `MCPToolset(...)`; there's no YAML wiring yet.
