...
the call site. The runtime mints the per-request token (attesting every role in
`user_roles`), picks the `billing` role's policy file, and evaluates its
constraints against each tool call. Pass more than one role and each is evaluated
— see [most permissive wins](#multiple-roles-most-permissive-wins).
...
platform, stored on `policy_decision` for **90 days**, and shown verbatim in the
...
role that granted it is recorded on the decision (`granted by:` in `hexgate
chat`, and in the audit trail).
...
grants something no named role grants; run it with
`--max-severity warning` in CI to make that a hard failure.
</Warning>
