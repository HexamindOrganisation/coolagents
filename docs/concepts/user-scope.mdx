...
Redaction is **key-name-only**, and applies to the audit copy alone: keys
matching `password|passwd|secret|token|api[-_]?key|credential|authorization`
become `"[REDACTED]"`. Values sensitive by *content* — email addresses, customer
or patient identifiers, free text — pass through unchanged. Prefer the coarsest
