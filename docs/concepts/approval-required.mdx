...
| `deciding_role` | `str \| None` | The role whose policy gated the call — the one the approver is deciding for. `None` when no role granted it. |
| `user_roles` | `tuple[str, ...]` | Every role the caller carried, in order. Empty when no scope was active. |
