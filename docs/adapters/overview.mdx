...
Every wrapper also takes [`guards`](/concepts/guards) — before/after-tool functions
that observe, rewrite arguments, or refuse a call — installed with `guards=[...]`
(or `HexgateRunner(guards=[...])` on the OpenAI and Google runners).
