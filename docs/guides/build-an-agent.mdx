...
record holds the YAML — the dev's code vs the dashboard.
...
- **Hosted (default):** set `HEXGATE_API_KEY` to the key from [app.hexgate.ai](https://app.hexgate.ai). Leave `HEXGATE_API_URL` unset.
...
- registering it with `register_agent(...)` and loading it through `load_agent(...)`
...
## Builtin and local agents

The package ships a small `hexgate.builtin_agents` directory for official starter
agents (currently `researcher`):
...
The CLI also discovers local agents from `./<agent_dir>/agent.yaml`,
