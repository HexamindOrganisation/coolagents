...
def test_on_llm_end_emits_usage_from_standardized_metadata(
...
def test_on_llm_end_falls_back_to_legacy_token_usage(
    emitted: list[dict[str, Any]],
) -> None:
...
def test_on_llm_end_does_nothing_when_no_usage_reported(
    emitted: list[dict[str, Any]],
) -> None:
    handler = HexgateUsageCallbackHandler(agent_name="my-agent", api_key="k")
